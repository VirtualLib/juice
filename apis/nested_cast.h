...
#ifndef VIRTUALLIB_JUICE_HIERARCHY_CAST_INCLUDE_H_ 
#define VIRTUALLIB_JUICE_HIERARCHY_CAST_INCLUDE_H_ 
...
inline void evaluate(const std::function<void(ARGS...)>& func, ARGS... args) {
    if (!func) { return; } return func(std::forward<Params>(args)...);
}

template<typename R, typename... ARGS>
inline R evaluate(const std::function<R(ARGS...)>& func, ARGS... args) {
    if (!func) { return R(); } return func(std::forward<Params>(args)...);
}
...
//    dynamic_cast<void*>(T*)
//    dynamic_cast<Parent*>(dynamic_cast<Parent*>(Child*)) = dynamic_cast<void*>(Child*)
//    
...
public:
    ...
    operator T*() const { return ptr_; }                        // dynamic_cast<T*>(u)
    operator void*() const { return nested_static_cast(ptr_); } // static_cast<void*>(u) || nested_cast
...
public:
    ...
    operator T*() const { return ptr_; }                        // dynamic_cast<T*>(u)
    operator void*() const { return dynamic_cast<void*>(ptr_); }  // dynamic_cast<void*>(u)
private:
    T * ptr_ = nullptr;
};
...
#endif  // !#define (VIRTUALLIB_JUICE_HIERARCHY_CAST_INCLUDE_H_ )