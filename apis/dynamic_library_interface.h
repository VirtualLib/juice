...
#ifndef VIRTUALLIB_JUICE_DYNAMIC_LIBRARY_INTERFACE_INCLUDE_H_
#define VIRTUALLIB_JUICE_DYNAMIC_LIBRARY_INTERFACE_INCLUDE_H_
...
public:
    ...
    NativeInterface* get() const {
        if (!library_name_.empty() && !x::WellKnownLibrary(library_name_)) return nullptr;
...
public:
    ...
    void Reset(const std::shared_ptr<DynamicLibrary>& library, const std::string& name) {
        ...
        function_ = x::GetFunctionPointer<R, P...>(library, name);
...
#endif // !VIRTUALLIB_JUICE_DYNAMIC_LIBRARY_INTERFACE_INCLUDE_H_