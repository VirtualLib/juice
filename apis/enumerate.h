...
#ifndef VIRTUALLIB_JUICE_ENUMERATE_INCLUDE_H_ 
#define VIRTUALLIB_JUICE_ENUMERATE_INCLUDE_H_ 
...
std::map<Type, std::string> parse(std::string input) {
    ...
    auto removed = x::RemoveChars(input, " ()\r\t\n", &input);
    auto size = x::Tokenize(input, ',', &tokens);   // Token: [Name | Name=EnumValue]
#ifdef _DEBUG
    ...
    for (auto iter = tokens.begin(); iter != tokens.end(); ++iter) {
        ...
        if (iter->find('=') != std::string::npos) {
            ...
            auto size = x::Tokenize(*iter, '=', &pair);
#ifdef _DEBUG
...
#define DECLARE_ENUM_WITH_TYPE(Name, Type, ...)                                             \
    ...
    static std::map<Type, std::string> Name##NamedMap(x::parse<Type>(#__VA_ARGS__));        \
...
#endif  // !#define (VIRTUALLIB_JUICE_ENUMERATE_INCLUDE_H_ )