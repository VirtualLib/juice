...
#ifndef VIRTUALLIB_JUICE_DYNAMIC_LIBRARY_INCLUDE_H_
#define VIRTUALLIB_JUICE_DYNAMIC_LIBRARY_INCLUDE_H_
...
struct FunctorTraits { using Type = R(*)(P...); };

class JUICE_API DynamicLibrary {
public:
    explicit DynamicLibrary() {}
    explicit DynamicLibrary(const HMODULE& library) : library_(library) {}
    explicit DynamicLibrary(const std::wstring& path) : library_(x::LoadLibrary(path, nullptr)) {}
    explicit DynamicLibrary(const wchar_t filename[]) : library_name_(filename) {}
...
#endif // !VIRTUALLIB_JUICE_DYNAMIC_LIBRARY_INCLUDE_H_